...
namespace KamiLib.Atk
{
    public unsafe class BaseNode
    {
        ...
        {
            node = (AtkUnitBase*)Service.GameGui.GetAddonByName(addon).Address;
        }
        ...
        {
            return node == null ? null : node->RootNode;
        }
    ...
    public unsafe class ComponentNode
    {
        ...
        public T* GetNode<T>(uint id) where T : unmanaged
        {
            return componentBase == null ? null : Node.GetNodeByID<T>(componentBase->UldManager, id);
        }
