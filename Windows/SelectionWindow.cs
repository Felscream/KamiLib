...
namespace KamiLib.Windows
{
    public abstract class SelectionWindow : Window, IDrawable
    {
        ...
        protected SelectionWindow(
            string windowName, float height = 0.0f, float initialSelectionWidth = 250.0f) : base(windowName)
        {
        ...
        public override void Draw()
        {
            ...
            if (ImGui.BeginTable($"{KamiCommon.PluginName}TableContainer", 2, ImGuiTableFlags.Resizable))
            {
                ...
                if (ImGui.BeginChild($"###{KamiCommon.PluginName}LeftSide", regionSize with { Y = topLeftSideHeight },
                                     ShowBorders, ImGuiWindowFlags.NoDecoration))
                {
                    selectionList.Draw(GetSelectables());
                }
                ImGui.EndChild();

                if (ImGui.BeginChild($"###{KamiCommon.PluginName}BottomLeftSide",
                                     regionSize with { Y = verticalHeight * ImGuiHelpers.GlobalScale }, ShowBorders,
                ...
                ImGui.TableNextColumn();
                if (ImGui.BeginChild($"###{KamiCommon.PluginName}RightSide", Vector2.Zero, ShowBorders,
                                     (ShowScrollBar ? ImGuiWindowFlags.AlwaysVerticalScrollbar : ImGuiWindowFlags.None)
                                     | ImGuiWindowFlags.NoDecoration))
                {
