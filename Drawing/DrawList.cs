    ...
    public abstract class DrawList<T> where T : DrawList<T>
    {
        ...
        public T AddIcon(uint iconID, Vector2 size, float transparency)
        {
            ...
            if (icon != null)
            {
                DrawActions.Add(() =>
                {
                    ImGui.Image(icon.Handle, size, Vector2.Zero, Vector2.One,
                                Vector4.One with { W = transparency });
                });
            }
        ...
        public T AddStringCentered(string message, float? availableArea = null, Vector4? color = null)
        {
            if (color == null)
            {
                DrawActions.Add(() =>
                {
                    ...
                    {
                        X = ImGui.GetCursorPos().X + area - ImGui.CalcTextSize(message).X / 2.0f,
                    });
                    ImGui.Text(message);
                });
            }
        ...
        public T AddConfigCheckbox(
            string label, Setting<bool> setting, string? helpText = null, string? additionalID = null)
        {
            DrawActions.Add(() =>
            {
        ...
        public T AddConfigCombo<TU>(
            IEnumerable<TU> values, Setting<TU> setting, Func<TU, string> localizeFunction, string label = "",
            float width = 0.0f) where TU : struct
        {
            DrawActions.Add(() =>
            {
                if (width != 0.0f)
        ...
        public T AddConfigColor(string label, Setting<Vector4> setting)
        {
            DrawActions.Add(() =>
            {
                if (ImGui.ColorEdit4(label, ref setting.Value,
        ...
        public T AddDragFloat(
            string label, Setting<float> setting, float minValue, float maxValue, float width = 0.0f, int precision = 2)
        {
            DrawActions.Add(() =>
            {
                if (width != 0.0f)
        ...
        public T AddConfigRadio<TU>(string label, Setting<TU> setting, TU buttonValue, string? helpText = null)
            where TU : struct
        {
            DrawActions.Add(() =>
            {
        ...
        public T AddInputInt(
            string label, Setting<int> settingsPriority, int min, int max, int step = 1, int stepFast = 1,
            float width = 77.0f)
        {
            DrawActions.Add(() =>
            {
        ...
        public T AddInputString(
            string label, Setting<string> setting, int maxLength, ImGuiInputTextFlags flags = ImGuiInputTextFlags.None)
        {
            DrawActions.Add(() =>
            {
        ...
        public T AddDisabledButton(
            string label, Action action, bool disable, string? hoverTooltip = null, float? buttonSize = null)
        {
        ...
        private void AddDisabledButtonWithSize(
            string label, Action action, bool disable, float buttonSize, string? hoverTooltip = null)
        {
            DrawActions.Add(() =>
            {
        ...
        private void AddDisabledButtonWithoutSize(
            string label, Action action, bool disable, string? hoverTooltip = null)
        {
            DrawActions.Add(() =>
            {
                if (disable) ImGui.PushStyleVar(ImGuiStyleVar.Alpha, 0.5f);
