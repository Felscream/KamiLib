...
namespace KamiLib.ZoneFilterList
{
    public static class ZoneFilterListDraw
    {
        ...
        public static void DrawFilterTypeRadio(Setting<ZoneFilterTypeId> currentType)
        {
            ...
            InfoBox.Instance
                   .AddTitle(Strings.ZoneFilterList_Type_Section, 1.0f)
                   .AddConfigRadio(Strings.ZoneFilterList_Type_AllowLabel, currentType, ZoneFilterType.WhiteList.Id,
                                   Strings.ZoneFilterList_Type_AllowTooltip)
                   .SameLine()
                   .AddConfigRadio(Strings.ZoneFilterList_Type_BlockLabel, currentType, ZoneFilterType.BlackList.Id,
                                   Strings.ZoneFilterList_Type_BlockTooltip)
                   .Draw();
        }
            ...
            InfoBox.Instance
                   ...
                   .AddDisabledButton(
                       EntriesToRemove.Count == 0
                           ? zoneFilterType.ClearButton
                           : Strings.ZoneFilterList_RemoveSelectedZones.Format(EntriesToRemove.Count), () =>
                       {
        ...
        public static void DrawAddRemoveHere(Setting<List<uint>> selectedZones)
        {
            InfoBox.Instance
                   ...
                   .AddAction(() => LuminaCache<TerritoryType>.Instance.GetRow(Service.ClientState.TerritoryType)
                                                              ?.DrawLabel())
                   .BeginTable()
                   .BeginRow()
                   .AddDisabledButton(Strings.Common_Add,
                                      () => { Add(selectedZones, Service.ClientState.TerritoryType); },
                                      selectedZones.Value.Contains(Service.ClientState.TerritoryType),
                                      buttonSize: InfoBox.Instance.InnerWidth / 2.0f - 5.0f * ImGuiHelpers.GlobalScale)
        ...
        public static void DrawTerritorySearch(Setting<List<uint>> selectedZones, ZoneFilterType zoneFilterType)
        {
            InfoBox.Instance
                   ...
                   .AddAction(() =>
                   {
                       ...
                       if (ImGui.InputTextWithHint("###TerritorySearch", Strings.ZoneFilterList_Search,
                                                   ref _searchString,
                                                   60, ImGuiInputTextFlags.AutoSelectAll))
                       {
        ...
        private static List<SearchResult> Search(string searchTerms, int numResults)
        {
            return Service.DataManager.GetExcelSheet<TerritoryType>()!
                          ...
                          .Where(territory => territory.PlaceName.Value!.Name.ToDalamudString().TextValue.ToLower()
                                                       .Contains(searchTerms.ToLower()))
        ...
        private static void DisplayResults(List<SearchResult>? results)
        {
            ...
            if (ImGui.BeginChild("###SearchResultsChild",
                                 new Vector2(InfoBox.Instance.InnerWidth, 21.0f * 5 * ImGuiHelpers.GlobalScale)))
            {
                foreach (var result in results)
                {
                    if (ImGui.Selectable($"###SearchResult{result.TerritoryID}",
                                         EntriesToAdd.Contains(result.TerritoryID)))
                    {
        ...
        private static void SelectedZonesList(Setting<List<uint>> selectedZones, ZoneFilterType zoneFilterType)
        {
            ...
            if (ImGui.BeginChild("###ZoneFilterListFrame", size))
            {
                if (!selectedZones.Value.Any())
                {
                    ImGui.SetCursorPos(ImGui.GetCursorPos() with
                    {
                        X = ImGui.GetContentRegionAvail().X / 2 -
                            ImGui.CalcTextSize(zoneFilterType.IsEmptyLabel).X / 2.0f,
                    });
